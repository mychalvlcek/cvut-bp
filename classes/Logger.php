...
final class Logger {
	...
	public final static function errorHandler( $errNo, $errStr, $errFile, $errLine, $errContext = null ) {
		//vynechám striktní chyby
		if ( $errNo == E_STRICT ) {
			return false;
		}
		// okamžitý výpis chyby na screen X logování chyby do souboru
		if ( self::$debug ) {
			//nastavení barev
			...
			//vypsání
			echo '<div style="z-index: 255; border: 3px solid #'.$errcolor.'; border-radius: 5px; padding: 10px; background-color: rgba(245,245,245,0.5); font-size: 13px; color: #000000;opacity: 0.8">'
			...
			if ( $errNo != E_NOTICE && $errNo != E_USER_NOTICE ) {
				...
				//odeslání do logu
				error_log($msg."\n\n".str_repeat("-", 100)."\n\n", 3, ABS_PATH."log/".date("Y-m-d")."-php.txt");
			}
			//v případě fatal error, ukončím provádění
			if ( $errNo == E_ERROR || $errNo == E_USER_ERROR ) {
				if ( defined("web_fail_url") && strlen(web_fail_url) > 10 ) {
					header("Location: ".web_fail_url);
				} else {
					header("HTTP/1.1 500 Internal Server Error");
				}
				exit();
	...
	/**
	 * Zaregistrovaná shutdown funkce
	 * Funkce se volá automaticky PHP interpreterem
	 * Nevolat přímo!!!
	 */
	public final static function shutdownHandler( ) {
		//pokud skript skončí s chybou
		if ( $error = error_get_last() ) {
			if ( preg_match("%^".ABS_PATH."%", $error['file']) ) {
				self::errorHandler($error['type'], $error['message'], $error['file'], $error['line']);
			}
		}
		//pokud processTime vykonávání je dlouhá
	...
	/**
	 * Returns microtime for time process tracking
